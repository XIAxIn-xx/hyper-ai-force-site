      ...
      <div className="section-shell relative z-10">
        ...
        <Reveal delay={0.1}>
          <div className="relative mt-20 grid border-t border-white/25 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rail-scan pointer-events-none absolute left-0 right-0 top-0 h-px bg-gradient-to-r from-transparent via-cyan-200 to-transparent" />
            {content.market.points.map((point, index) => (
              <div key={point} className="group relative overflow-hidden border-b border-white/15 py-6 transition-colors duration-300 hover:border-cyan-200/50 sm:border-r sm:px-6 sm:first:pl-0 lg:border-b-0 lg:last:border-r-0">
                <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-cyan-300/10 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                <span className="relative flex items-center gap-2 text-xs font-semibold tracking-[0.2em] text-cyan-200">
                  <span className="h-1.5 w-1.5 bg-cyan-200 transition-transform duration-300 group-hover:scale-150" />
                  0{index + 1}
                </span>
