...
export function ServiceNetworkSection({ content }: ServiceNetworkSectionProps) {
  ...
  return (
    <section id="collaboration" className="bg-[#FAF9F7] py-20 md:py-24">
          ...
          <div className="mx-auto mt-12 grid max-w-[1008px] gap-6 md:grid-cols-3">
            {serviceItems.map((item, index) => {
              ...
              const descriptionItems = item.description
                ...
                .filter(Boolean);
              return (
                <article key={item.label} className="group relative min-h-[190px] overflow-hidden rounded-xl border border-[#DDE4EC] bg-white p-6 shadow-[0_12px_34px_rgba(15,23,42,0.05)] transition duration-500 hover:-translate-y-1 hover:border-orange-300 hover:shadow-[0_18px_42px_rgba(249,115,22,0.14)]">
                  ...
                  <div className="relative flex items-start justify-between gap-4">
                    <Icon className="h-10 w-10 text-orange-600 transition duration-500 group-hover:-translate-y-1 group-hover:scale-110" />
                  ...
                  <ul className="zh-copy relative mt-3 list-disc space-y-1 pl-5 text-sm leading-7 text-slate-600 marker:text-orange-500">
